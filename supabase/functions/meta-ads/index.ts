    ...
    switch (action) {
      ...
      case "get_ads": {
        ...
        const adInsightsTR = dateRange?.from && dateRange?.to
          ...
          : "";
        const adFields =
          `name,status,creative{thumbnail_url,image_url,body,title,link_url,call_to_action_type,object_story_spec},adset{name,targeting},insights${adInsightsTR}{spend,impressions,clicks,actions,purchase_roas,ctr,cpc}`;
        const data = await metaFetchAll(
